...
for exp in ALICE PHENIX CMS ATLAS STAR
do
    ...
    filename31="StrangeParticles_dNdy.dat"
    ...
    (
        ...
        echo "# cen dN/dy dN/dy_err(Lambda, Lambdabar, Xi^-, Xibar^+, Omega, Omegabar, phi)" > $filename31
        ...
        do
            ...
            v22=`cat ./$icen/charged_hadron_integrated_observables.dat | sed 's/phenix/PHENIX/g' | sed 's/star/STAR/g' | grep "$exp" | grep "v_2" | awk {'print $2, $4'}`
            v32=`cat ./$icen/charged_hadron_integrated_observables.dat | sed 's/phenix/PHENIX/g' | sed 's/star/STAR/g' | grep "$exp" | grep "v_3" | awk {'print $2, $4'}`
            v42=`cat ./$icen/charged_hadron_integrated_observables.dat | sed 's/phenix/PHENIX/g' | sed 's/star/STAR/g' | grep "$exp" | grep "v_4" | awk {'print $2, $4'}`
            v52=`cat ./$icen/charged_hadron_integrated_observables.dat | sed 's/phenix/PHENIX/g' | sed 's/star/STAR/g' | grep "$exp" | grep "v_5" | awk {'print $2, $4'}`
            ...
            # Strangeness
            Lambda=`cat ./$icen/Lambda_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            Lambdabar=`cat ./$icen/anti_Lambda_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            Xi_m=`cat ./$icen/Xi_m_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            Xibar_p=`cat ./$icen/anti_Xi_p_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            Omega=`cat ./$icen/Omega_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            Omegabar=`cat ./$icen/anti_Omega_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            phi=`cat ./$icen/phi_integrated_observables.dat | grep "dN/dy" | cut -f 2,4 -d " "`
            echo ${centrality[idx]} $Lambda $Lambdabar $Xi_m $Xibar_p $Omega $Omegabar $phi >> $filename31
